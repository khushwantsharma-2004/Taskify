...
export default function Layout({ children }) {
  ...
  return <div className="layout"><aside className={`sidebar${menuOpen ? " menu-open" : ""}`}><div className="brand"><span className="brand-mark">T</span><span>Taskify</span></div><button className="mobile-menu" type="button" aria-label={menuOpen ? "Close navigation" : "Open navigation"} aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}>{menuOpen ? "Close" : "Menu"}</button><p className="sidebar-kicker">Your study command center</p><nav className={`nav${menuOpen ? " open" : ""}`} onClick={() => setMenuOpen(false)}>{links.map(([to, label]) => <NavLink key={to} to={to} className={({ isActive }) => isActive ? "active" : ""}>{label}</NavLink>)}</nav><button className="logout ghost" onClick={logout}>Log out</button></aside><main className="main"><header className="topbar"><div className="topbar-copy"><span className="eyebrow">Student workspace</span><h1>{links.find(([to]) => location.pathname.startsWith(to))?.[1] || "Dashboard"}</h1></div><strong className="topbar-name">{user?.name || "Student"}</strong></header><div className="content">{children}</div></main></div>;
}